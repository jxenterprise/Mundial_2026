...
(function () {
  ...
  function urlCompartir(p) {
    ...
    if (p.estado === "fin") {
      texto = `⚽ Mundial 2026 · ${DATOS_MUNDIAL.nombresFase[p.fase] || ""}\n` +
              `${nombreDe(p, "L")} ${p.gl} – ${p.gv} ${nombreDe(p, "V")}` +
      ...
      texto = `⚽ Mundial 2026 · ${DATOS_MUNDIAL.nombresFase[p.fase] || ""}\n` +
              `${nombreDe(p, "L")} vs ${nombreDe(p, "V")} — ${fechaHoraLocal(p)} (tu hora local)\n` +
  ...
  function lineaEquipo(p, lado) {
    ...
    return `
      <div class="cp-linea">
        <span class="cp-bandera" aria-hidden="true">${banderaHTML(code, "b-md")}</span>
  ...
  function renderHero() {
    ...
    cont.innerHTML = `
      <div class="marcador" data-id="${p.id}">
        ...
        <div class="m-fila">
          <div class="m-equipo">
            <span class="m-bandera" aria-hidden="true">${banderaHTML(p.local, "b-xl")}</span>
            <span class="m-nombre">${nombreDe(p, "L")}</span>
          </div>
          <div class="m-centro">${centro}</div>
          <div class="m-equipo">
            <span class="m-bandera" aria-hidden="true">${banderaHTML(p.visita, "b-xl")}</span>
